...
public class IllagerEyeItem extends Item {
    public IllagerEyeItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        HitResult hitResult = rayTrace(world, user, RayTraceContext.FluidHandling.NONE);
            ...
            if (world instanceof ServerWorld) {
                BlockPos blockPos = ((ServerWorld)world).getChunkManager().getChunkGenerator().locateStructure((ServerWorld)world, StructureFeature.MANSION, user.getBlockPos(), 100, false);
                if (blockPos != null) {
